...
+ [新增] 画布多图生成改为图片组框，按稳定顺序直接平铺全部结果，并支持单图预览、下载、收藏、删除与拖动排序。
+ [新增] 普通图片可拖入图片组框，组内图片也可拖出为独立节点，媒体、提示词、生成参数与收藏状态保持不变。
+ [修复] 资产插入图片时回退使用封面地址，避免带本地存储键的资产创建空白节点；资产搜索同时匹配完整提示词。
+ [修复] 节点信息将画布尺寸与图片/视频原始分辨率分开显示，图片组逐张显示各自像素尺寸，缺失分辨率时显示未知。
